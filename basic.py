...
class Dataset(object):
    ...
    def __init__(self, data, label=None, max_bin=255, reference=None,
                 ...
                 free_raw_data=True,isFromFile = False,shape = None):
        """
        Parameters
        ----------
        data : string/numpy array/scipy.sparse
    ...
    def _lazy_init(self, data, label=None, max_bin=255, reference=None,
                   ...
                   categorical_feature='auto', params=None, isfromfile = False):
        if data is None:
            self.handle = None
            return
        ...
        if isfromfile:
            self.__init_from_np2d2(data, self.shape[0], self.shape[1], params_str, ref_dataset)
        else:
            if isinstance(data, string_type):
    ...
    def __init_from_np2d2(self, filepath, shape0, shape1, params_str, ref_dataset):
        """
            ...
            if self.reference is not None:
                if self.used_indices is None:
                    ...
                    self._lazy_init(self.data, label=self.label, max_bin=self.max_bin, reference=self.reference,
                                    ...
                                    silent=self.silent, feature_name=self.feature_name, params=self.params,isfromfile=self.isFromFile)
                else:
                    """construct subset"""
                ...
                self._lazy_init(self.data, label=self.label, max_bin=self.max_bin,
                                ...
                                categorical_feature=self.categorical_feature, params=self.params,isfromfile=self.isFromFile)
